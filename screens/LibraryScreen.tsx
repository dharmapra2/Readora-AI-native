    ...
    >
      ...
      <View style={styles.copy}>
        ...
        <View style={styles.rowActions}>
          ...
          <TouchableOpacity onPress={() => { onSelectBook(book.id); onOpen("bookDetail"); }}>
...
export function LibraryScreen({
  ...
  const filteredBooks = useMemo(() => {
    ...
    return books.filter((book) => {
      const matchesQuery = !q || book.title.toLowerCase().includes(q) || book.author.toLowerCase().includes(q) || book.chapterTitle.toLowerCase().includes(q);
      return matchesQuery;
    });
  }, [books, query]);
  ...
  const renderItem = useCallback<ListRenderItem<Book>>(
    ({ item }) => (
      <BookRow
        book={item}
        onOpen={onOpen}
        onRemoveBook={onRemoveBook as (id: string) => void}
        onSelectBook={onSelectBook}
        onUpdateProgress={onUpdateProgress as (id: string, progress: number) => void}
      />
    ),
    [onOpen, onRemoveBook, onSelectBook, onUpdateProgress],
  );
...
function FormInput({ keyboardType, label, onChangeText, placeholder, value }: {
...
const styles = StyleSheet.create({
  ...
  addTopButton: { width: 44, height: 44, borderRadius: 22, backgroundColor: colors.purple, alignItems: "center", justifyContent: "center" },
  row: { ...shadow, flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 14, padding: 12, borderRadius: 16, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border },
  copy: { flex: 1 },
  title: { color: colors.ink, fontSize: 15, fontWeight: "900" },
  author: { color: colors.muted, fontSize: 12, marginTop: 4 },
  chapter: { color: "#596074", fontSize: 12, marginTop: 4 },
  progressTrack: { marginTop: 11, height: 5, borderRadius: 3, backgroundColor: "#E5E7F0", overflow: "hidden" },
  progressFill: { height: "100%", borderRadius: 3, backgroundColor: colors.purple },
  rowActions: { flexDirection: "row", gap: 14, marginTop: 10 },
  actionText: { color: colors.purple, fontSize: 12, fontWeight: "800" },
  removeText: { color: "#EF4444", fontSize: 12, fontWeight: "800" },
  progressText: { color: colors.ink, fontSize: 12, fontWeight: "800" },
  emptyState: { alignItems: "center", justifyContent: "center", paddingVertical: 54 },
  emptyTitle: { color: colors.ink, fontSize: 18, fontWeight: "900", marginTop: 12 },
  emptyCopy: { color: colors.muted, fontSize: 14, marginTop: 8, textAlign: "center" },
  modalScrim: { flex: 1, justifyContent: "flex-end", backgroundColor: "rgba(17,19,42,0.32)" },
  modal: { backgroundColor: colors.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 22, paddingBottom: 34 },
  modalHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 18 },
  modalTitle: { color: colors.ink, fontSize: 22, fontWeight: "900" },
  formGroup: { marginBottom: 14 },
  formLabel: { color: colors.ink, fontSize: 13, fontWeight: "800", marginBottom: 8 },
  formInput: { minHeight: 52, borderRadius: 14, backgroundColor: "#F1F3F8", color: colors.ink, paddingHorizontal: 14, fontSize: 15 },
  saveButton: { minHeight: 56, borderRadius: 16, backgroundColor: colors.purple, alignItems: "center", justifyContent: "center", marginTop: 6 },
